...
float getElevation(vec3 position)
{
    ...
    float elevation = 0.0;
    elevation += cnoise(warpedPosition * uPositionFrequency      ) / 2.0;
...
void main()
{
    ...
    positionA.y    += getElevation(positionA);
    positionB.y    += getElevation(positionB);
    ...
    // Varyings
    vPosition = vec3(getElevationColor(csm_Position));
    // warp vPosition with cnoise
